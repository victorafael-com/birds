...
// Type: UnityEngine.EventSystems.VRPointerInputModule
...
namespace UnityEngine.EventSystems
{
    ...
    /// </summary>
    public abstract class VRPointerInputModule : BaseInputModule
    {
        protected Dictionary<int, PointerEventData> m_PointerData = new Dictionary<int, PointerEventData>();
        private readonly VRPointerInputModule.MouseState m_MouseState = new VRPointerInputModule.MouseState();
        /// <summary>
        ...
        /// </summary>
        /// <param name="id"></param>
        protected virtual VRPointerInputModule.MouseState GetMousePointerEventData()
        {
        ...
        /// </summary>
        /// <param name="id"></param>
        protected virtual VRPointerInputModule.MouseState GetMousePointerEventData(int id)
        {
            PointerEventData data1;
            bool pointerData = this.GetPointerData(-1, out data1, true);
            data1.Reset();
			Vector2 pos = VRInputModule.CenterPos;

			if (pointerData)
				data1.position = pos;// new Vector2(ConstHelper.INPUT_CENTER_Y, ConstHelper.INPUT_CENTER_Y);

			Vector2 mousePosition = pos;//new Vector2(ConstHelper.INPUT_CENTER_Y, ConstHelper.INPUT_CENTER_Y);
            ...
            this.m_MouseState.SetButtonState(PointerEventData.InputButton.Left, VRPointerInputModule.StateForMouseButton(0), data1);
            this.m_MouseState.SetButtonState(PointerEventData.InputButton.Right, VRPointerInputModule.StateForMouseButton(1), data2);
            this.m_MouseState.SetButtonState(PointerEventData.InputButton.Middle, VRPointerInputModule.StateForMouseButton(2), data3);
        ...
        protected virtual void ProcessDrag(PointerEventData pointerEvent)
        {
            ...
            if (flag && pointerEvent.pointerDrag != null && (!pointerEvent.dragging && ShouldStartDrag(pointerEvent.pressPosition, pointerEvent.position, (float) this.eventSystem.pixelDragThreshold, pointerEvent.useDragThreshold)))
            {
            ...
            if (!pointerEvent.dragging || !flag || !(pointerEvent.pointerDrag != null))
                return;
            if (pointerEvent.pointerPress != (Object) pointerEvent.pointerDrag)
            {
        ...
        protected void ClearSelection()
        {
            ...
            using (Dictionary<int, PointerEventData>.ValueCollection.Enumerator enumerator = this.m_PointerData.Values.GetEnumerator())
            {
                while (enumerator.MoveNext())
                    this.HandlePointerExitAndEnter(enumerator.Current, null);
            }
        ...
        {
            if (!(ExecuteEvents.GetEventHandler<ISelectHandler>(currentOverGo) != (Object) this.eventSystem.currentSelectedGameObject))
                return;
            eventSystem.SetSelectedGameObject((GameObject) null, pointerEvent);
        }

        protected class ButtonState
        {
            private PointerEventData.InputButton m_Button;
            private VRPointerInputModule.MouseButtonEventData m_EventData;

            public VRPointerInputModule.MouseButtonEventData eventData
            {
                get
                {
        ...
        {
            private List<VRPointerInputModule.ButtonState> m_TrackedButtons = new List<VRPointerInputModule.ButtonState>();
            ...
            public ButtonState GetButtonState(PointerEventData.InputButton button)
            {
                ...
                if (buttonState == null)
                {
                    buttonState = new VRPointerInputModule.ButtonState()
                    {
                        button = button,
                        eventData = new VRPointerInputModule.MouseButtonEventData()
                    };
            ...
            {
                VRPointerInputModule.ButtonState buttonState = this.GetButtonState(button);
